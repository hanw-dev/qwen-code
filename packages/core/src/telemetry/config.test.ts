...
describe('telemetry/config helpers', () => {
  ...
  describe('parseTelemetryTargetValue', () => {
    ...
    it('returns undefined for non-local values', () => {
      expect(parseTelemetryTargetValue('gcp')).toBeUndefined();
      expect(parseTelemetryTargetValue(TelemetryTarget.GCP)).toBeUndefined();
  ...
  describe('resolveTelemetrySettings', () => {
    ...
    it('uses env over settings and argv over env', async () => {
      ...
      const env = {
        ...
        GEMINI_TELEMETRY_TARGET: 'gcp', // Will be rejected - only local allowed
      ...
      // Should throw because gcp target is not allowed
      await expect(resolveTelemetrySettings({ env, settings })).rejects.toThrow(
